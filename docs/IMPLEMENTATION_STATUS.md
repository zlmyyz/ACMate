...
> Updated: 2026-07-30 | 训练计划成员进度高优先级验收完成（V15/CF ACCEPTED/成员生命周期/截止冻结/备注只读）| 后端 541 测试 0 失败 前端 197 测试 0 失败 | 修复：rejoin 清除审计字段、管理员查看成员进度权限 | 截止冻结 10 项全部验证通过
...
| 训练成员进度 | 加入/移除/状态/备注/成员进度详情/排名/计分题/截止时间语义/必做选做 | **完成** | **完成** | 23 service pass + 12 progress pass + 5 frontend | **Chromium 真实环境验收通过（状态切换/备注/必做选做/权限/成员生命周期/计划停用恢复）** | 赛时排名快照（后续） |
